...
<body class="bg-gray-200">
                    ...
                    <section class="p-8 antialiased md:py-16">
                        <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
                            <div class="mb-4 flex items-center justify-between gap-4 md:mb-8">
                                ...
                                <button id="add_categorie_button" class="text-gray-100 bg-gray-900 hover:bg-gray-700 p-3 mb-5 mr-5 rounded-sm">Add Category</button>
                            </div>
                            ...
                            <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                                <?php foreach ($categories as $category): ?>
                                    <div class="category_box flex flex-col rounded-lg border border-gray-200 bg-white px-4 py-4 hover:bg-gray-50" data-category-id="<?= $category['id_categorie']?>">
                                        <h3 class="text-xl font-semibold text-gray-900 sm:text-2xl text-center"><?= htmlspecialchars($category['nom_categorie']) ?></h3>
                                        <div class="flex justify-between my-4">
                                            <h3 class="font-semibold text-gray-900">Subcategories:</h3>
                                            <button type="button" class="add_sub_cat" title="Add subcategory">
                                                ...
                                                <?php foreach ($category['sous_categories'] as $subCategory): ?>
                                                    <div class="sub_cat_box ml-2 border-2 border-gray-200 px-2 py-1 mb-2 rounded-lg flex justify-between" data-sub-category-id="<?= $subCategory['id_sous_categorie'] ?>">
                                                        ...
                                                        <div class="flex">
                                                            ...
                                                            <form action="/admin/handleCategories" method="POST" class="inline">
                                                                <input type="hidden" name="id_sub_categorie" value="<?= $subCategory['id_sous_categorie'] ?>">
                                        ...
                                        <div class="flex justify-between items-end mt-auto mx-2 pt-5">
                                            ...
                                            <form action="/admin/handleCategories" method="POST" class="inline">
                                                ...
                                                <input type="submit" value="Delete" name="delete_categorie" class="cursor-pointer">
                                            </form>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
    ...
    <div id="categorie_modal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center">
        <div id="add_modal_content" class="flex flex-col w-11/12 md:w-5/12 overflow-y-auto scrollbar-hidden mx-auto mt-10 p-4 bg-gray-200 rounded-sm shadow-lg">
            <div class="flex justify-between">
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl" id="modal_title">Add Category</h1>
                <button id="close_categorie_modal" class="flex justify-end items-center mb-4 float-right text-xl">&times;</button>
            </div>
            
            <!-- Category Form -->
            <form method="POST" action="/admin/handleCategories" id="category_form" class="mt-[25%] md:px-10 hidden">
                <div class="flex w-full">
                    <label for="category_name_input" class="text-gray-900 font-semibold w-1/3">Category Name:</label>
                    <input type="hidden" name="category_id_input" value="0" id="category_id_input">
                    <input type="text" name="category_name_input" id="category_name_input" value="" class="w-2/3" required>
                </div>
                <div class="flex justify-evenly">
                    <input type="submit" name="add_modify_category" class="text-gray-100 bg-gray-700 border-2 border-gray-700 hover:bg-gray-900 px-8 py-1 mt-20 mr-6 float-right rounded-sm cursor-pointer" value="Save">
                </div>
            </form>

            <!-- Subcategory Form -->
            <form method="POST" action="/admin/handleCategories" id="sub_category_form" class="mt-[25%] md:px-10 hidden">
                <div class="flex w-full">
                    <label for="subcategory_name_input" class="text-gray-900 font-semibold w-1/3">Subcategory Name:</label>
                    <input type="hidden" name="category_parent_id_input" value="0" id="category_parent_id_input">
                    <input type="hidden" name="subcategory_id_input" value="0" id="subcategory_id_input">
                    <input type="text" name="subcategory_name_input" id="subcategory_name_input" value="" class="w-2/3" required>
                </div>
                <div class="flex justify-evenly">
                    <input type="submit" name="add_modify_subcategory" class="text-gray-100 bg-gray-700 border-2 border-gray-700 hover:bg-gray-900 px-8 py-1 mt-20 mr-6 float-right rounded-sm cursor-pointer" value="Save">
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        modifyCategoryButtons.forEach(button => {
            button.onclick = () => {
                modalTitle.textContent = 'Modify Category';
                showModal();
                document.getElementById('category_form').classList.remove("hidden");
                document.getElementById("category_id_input").value = button.closest(".category_box").getAttribute("data-category-id");
                document.getElementById("category_name_input").value = button.closest(".category_box").querySelector("h3").textContent.trim();
            }
        });
        ...
        addSubCategoryButtons.forEach(button => {
            button.onclick = () => {
                ...
                document.getElementById("category_parent_id_input").value = button.closest(".category_box").getAttribute("data-category-id");
            }
        });

        // Show modal as modify subcategory
        const modifySubCategoryButtons = document.querySelectorAll(".modify_sub_cat");
        modifySubCategoryButtons.forEach(button => {
            button.onclick = () => {
                modalTitle.textContent = 'Modify Subcategory';
                showModal();
                document.getElementById('sub_category_form').classList.remove("hidden");
                document.getElementById("category_parent_id_input").value = button.closest(".category_box").getAttribute("data-category-id");
                document.getElementById("subcategory_id_input").value = button.closest(".sub_cat_box").getAttribute("data-sub-category-id");
                document.getElementById("subcategory_name_input").value = button.closest(".sub_cat_box").querySelector("span").textContent.trim();
            }
        });
        ...
        function closeModal() {
            ...
            document.getElementById("subcategory_id_input").value = "0";
            document.getElementById("category_parent_id_input").value = "0";
        }
    </script>
</body>
</html>
